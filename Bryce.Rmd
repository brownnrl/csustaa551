...
###Analysis Section
The analysis to determine the best model to predict price using the ‘Seattle.csv’ dataset was to analyze the correlations of the predictors variables with price, and to gradually add predictor variables with the highest correlations to the model while assessing model performance. This analysis began with analyzing a correlogram of the dataset, excluding the variables id, date, waterfront, renovated and basement. It was determine id and date would not be helpful in our model to predict price because id does not give any information on the price of a house, and since we are looking at producing a model that can predict housing prices in Seattle in upcoming years, date would not provide us the type of information we need for future values. Waterfront, renovated and basement were indicator variables that were not needed in this visual analysis and will be introduced in the analysis later. 

Looking at the correlogram it was clear that the variables sqft_above, grade, sqft_living, bathrooms, and bedrooms had a direct relationship with our response variable price. This proved to be the case as we considered the correlations of the variables in descending order. Using the correlations of these predictors, we began to build our model with a sequential approach, beginning with the predictor with the highest correlation to price, sqft_living. With a p-value of < 2.2e-16, it was clear that we had strong evidence for a linear association between price and sqft_living. This simple linear regression model explained roughly 52% of the variation in price.

We then proceeded to add the variable with the second highest correlation to price, which was grade. Adding grade to the model proved to be helpful, as we noticed a significant p-value for the variables added last test in the summary table.

In the next step of building the model it is important to note that we did not attempt to add ‘sqft_above’ into the model since we determined this variable to be almost identical to ‘sqft_living’, with a correlation value of 0.898 to backup this assumption.

Next we added bathrooms into our model since it had a correlation of 0.575 with price. Adding bathrooms resulted in a p-value of 0.449107 for the variables added last t-test so it was safe to conclude that it was not helpful to our model with sqft_living and grade already applied.

Next we included view into our model and determined view to be helpful in our model, with sqft_living and grade already present. With three variables determine to explained roughly 59% of the variation in price, we looked to try to add bedrooms into the model. It was clear with the results from the summary table that bedrooms hardly explained any more variation in price, and was not helpful in our model so we removed that predictor.

At this point in our analysis we concluded to stop adding quantitative variables based off of their correlation to price since we wanted to conserve the parsimonious nature of the model. We did want to try the variable yr_built in the model since we believe it was important to attempt to add a variable that represented the age of the house. Adding yr_built into our model proved to be significant with sqft_living, grade and view already present in the model.

Next we looked to add indicator variables in our model and see if they would be helpful in explaining the variability in price. Starting with waterfront, we found that this indicator was helpful in our model and increased the amount of variation explained in price, so we decided to keep the variable in our model. 

It was determined that basement was not helpful in our model in two ways. We sequentially added basement to the model with waterfront already in the model and viewed a large p-value for the variables added last test. We also took waterfront out of the model and added basement to the model with sqft_living, grade and view and found it not to be helpful.

With the predictors sqft_living, grade, view and waterfront in our model, we are explaining 65% of the variability in price. At this point we are satisfied with these four predictors being used in our model.

Our next step in the analysis was to assess the collinearity in this model. When looking at the correlation values between the predictors, it was clear that we had evidence of collinearity between sqft_living and grade. With this knowledge we seeked to assess whether an interaction between these two variables would be helpful to our model and address this collinearity. Adding this interaction term proved to be helpful in our model and increased the variation in priced explained by the model by roughly 5%.

Satisfied with this model, we proceeded to check the diagnostic plots of this model. After review of the diagnostic plots, it was clear we had problems addressing our assumptions of linearity, mean zero for the errors and constant variance of the errors. This can be shown with the fanning out of the points in the residuals vs fitted plot, and the no linearity of the normal QQ plot. We can also see that we have observations with a very large Cook’s Distance, which may be negatively affecting our model. Checking the residuals versus the predicted values confirmed what we saw in the previous three plots and we concluded that our assumptions were violated.

To address the violations of our assumptions, we entertained the idea of transforming some of the variables used in the model. We could see from the correlogram that both our response, price, and predictor, sqft_living, had heavily right skewed distributions. We decided to apply a logarithmic transformation on both of these variables and reapply them into our model. Doing so we saw our interaction between the now logarithmic transformation of sqft_living and grade become not helpful to our model. We also, saw that the indicator, waterfront, was no longer helpful to the model. In an effort to satisfy our assumptions of the model, it was decided that removing both of these variables from the model was necessary.

The following model with log(price) ~ log(sqft_living) + grade + view + yr_built, we found all significant variables, and that we were able to explain roughly 64% of the variability in the log(price). Assessing the diagnostic plots for this model we found that our assumptions of linearity of the model, errors with mean of zero and constant variance in our residuals were satisfied. We proceeded to conclude this as our final model in predicting price.

In an effort to address the values that showed large Cook’s Distance in the diagnostic plots, we compared which leverage points in our model also had large residuals. We found observations that satisfied both these criteria, and concluded that these points needed to be addresses with the client. 
