...
The model with these three variables determine an adjusted $R^2$ which explained roughly 59% of the variation.  This model is  shown as model (2) in Table 2.  We next looked at adding `bathrooms` and `bedrooms` into the model based on the correlation with `price`.  `bedrooms` was chosen first as it had only a 0.428 correlation with `grade` versus the 0.740 correlation of `bathrooms` with `grade`. It was clear with the results from the summary table that bedrooms hardly explained any more variation in price, and was not considered a helpful addition in our model so we removed that predictor.  `bathrooms` showed a similar result and was not included in our analysis.
